...
class DefaultConfigs_ACARS(object):
    ''' 默认参数配置 '''
    # Dataset
...
class DefaultConfigs_RML(object):
    ...
    signal_len = "128,2"
    train_path = 'D:\WIN_10_DEDKTOP\onWorking\SAR&DD\PyTorch_Codes\Signal_master-UI\dataset\RML2016_04c\\6dB-SNR_50-samples.npy'  # 原始训练数据目录
    test_path = 'D:\WIN_10_DEDKTOP\onWorking\SAR&DD\PyTorch_Codes\Signal_master-UI\dataset\RML2016_04c\\6dB-SNR_4055-test.npy'    # 原始测试数据目录
    # train_path = 'D:\WIN_10_DEDKTOP\onWorking\SAR&DD\PyTorch_Codes\Signal_master\dataset\RML2018\\30dB-SNR_100-samples.npy'  # 原始训练数据目录
    # test_path = 'D:\WIN_10_DEDKTOP\onWorking\SAR&DD\PyTorch_Codes\Signal_master\dataset\RML2018\\30dB-SNR_500-test.npy'    # 原始测试数据目录
    process_IQ = True                           # 是否在载入数据时对IQ两路进行预处理

    batch_size = 4                            # DataLoader中batch大小，550/110=5 Iter
    num_workers = 4                             # DataLoader中的多线程数量

    # model
    model = "MsmcNet_RML2016"                   # 指定模型，目前就一个
    resume = True                              # 是否加载训练好的模型
    checkpoint_name = 'MsmcNet_RML2016_04c.pth' # 训练完成的模型名
